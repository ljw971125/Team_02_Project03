...
public class InquiryDAO extends JDBConnect {
	...
	// 검색 조건에 맞는 게시물의 개수를 반환합니다.
	  public int selectCount(Map<String, Object> map) {
	      ...
	      System.out.println(map.get("sessionNik"));
	      System.out.println(map.get("viewValue"));
	      ...
	      if ("admin".equals(map.get("sessionNik"))) {  // 현재 로그인한 사용자가 관리자일 경우 모든 문의사항(inquiry) 데이터를 가져옴
	          query += " WHERE nik is not null ";
	      }
	      else {  // 현재 로그인한 사용자가 관리자가 아닐 경우 해당 사용자가 작성한 문의사항 데이터만 가져옴
	          query += " WHERE nik='" + map.get("sessionNik") + "'";
	      }

	      if(map.get("viewValue") != null && !map.get("viewValue").equals("viewAll")) { // 전체보기가 아닐경우 미답변한 게시글만 출력
	      ...
	      try {
	          ...
	          System.out.println(totalCount); //값 없음
	      }
	      catch (Exception e) {
	...
	public List<InquiryDTO> selectList(Map<String, Object> map) {
		...
		try {
			...
			while (rs.next()) { // 결과를 순회하며...
				...
				dto.setIcomment(rs.getString("icomment")); // 답변
	...
	public int insertWrite(InquiryDTO dto) {
		...
		try {
			// INSERT 쿼리문 작성
			String query = "INSERT INTO inquiry ( " + " num,title,icontent,nik) " + " VALUES ( "
	...
	public int insertIcomment(InquiryDTO dto) {
		...
		try {
			// INSERT 쿼리문 작성
			String query = "UPDATE inquiry SET " + " icomment = ? " + " WHERE num=?";
	...
	public InquiryDTO selectView(String num) {
		...
		// 쿼리문 준비
		String query = "SELECT I.*, M.nik " + " FROM member1 M INNER JOIN inquiry I " + " ON M.nik=I.nik "
				+ " WHERE num=?";
	...
	public int updateEdit(InquiryDTO dto) {
		...
		try {
			// 쿼리문 템플릿
			String query = "UPDATE inquiry SET " + " title=?, icontent=? " + " WHERE num=?";

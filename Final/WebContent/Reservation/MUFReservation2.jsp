...
$(document).ready(function() {
    ...
    function sendSelectedData() {
        ...
        var selectedTimes = document.getElementById("selectedTimes").value;
        var price = document.getElementById("price").value;

        // AJAX 요청을 보내는 코드
        $.ajax({
            url: "your_server_endpoint", // 서버의 엔드포인트 URL을 여기에 입력하세요
            type: "POST", // 요청 방식을 선택하세요 (GET 또는 POST)
            data: {
                date: selectedDate,
                times: selectedTimes,
                price: price
            ...
            success: function(response) {
                ...
                // 추가적인 작업을 수행하거나 응답에 대한 처리를 진행하세요
            },
            error: function(xhr, status, error) {
                // 요청 실패 또는 응답 상태 코드가 실패를 나타낼 때 실행되는 코드

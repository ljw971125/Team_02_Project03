...
InquiryDTO dto = new InquiryDTO();             // DTO 객체 생성
InquiryDAO dao = new InquiryDAO(application);  // DAO 객체 생성
...
if (sessionId.equals(dto.getNik()) || sessionId.equals("admin")) {  // 작성자가 본인 혹은 관리자인지 확인 
    // 작성자가 본인이면...
    dto.setNum(num);
    ...
    if (delResult == 1) { 
        // 성공 시 목록 페이지로 이동
        JSFunction.alertLocation("삭제되었습니다.", "InquiryMain.jsp", out); 
    } else {
        // 실패 시 이전 페이지로 이동
